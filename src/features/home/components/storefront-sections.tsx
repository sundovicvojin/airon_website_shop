...
type StorefrontSectionsProps = Readonly<{ copy: Dictionary["home"]; locale: Locale }>;

export function StorefrontSections({ copy, locale }: StorefrontSectionsProps) {
  return <>
    <section className="home-collections"><Container><Reveal><p className="type-eyebrow text-accent">{copy.collections.eyebrow}</p><div className="home-section-heading"><h2>{copy.collections.title}</h2><p>{copy.collections.body}</p></div><div className="home-empty-row"><span>000</span><p>{copy.collections.empty}</p><ButtonLink href={`/${locale}/collections`} variant="quiet">{copy.collections.action}<ArrowIcon className="size-4" /></ButtonLink></div></Reveal></Container></section>
